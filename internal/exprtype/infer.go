...
// calls. schema and defs are shared across branches; guards is a shallow-copied
...
// withGuard returns a new inferCtx with path mapped to narrowed. schema and
...
func (c inferCtx) withGuard(path string, narrowed map[string]any) inferCtx {
	...
	return inferCtx{schema: c.schema, defs: c.defs, guards: guards}
}
...
func InferType(expression string, schema map[string]any) (map[string]any, error) {
	...
	return inferNode(tree.Node, inferCtx{schema: schema, defs: defs})
}
...
func inferNode(node ast.Node, ictx inferCtx) (map[string]any, error) {
	...
	case *ast.IdentifierNode:
		...
		return lookupProperty(ictx.schema, n.Value, ictx.defs)
	case *ast.MemberNode:
