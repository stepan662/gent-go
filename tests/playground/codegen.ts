// Generates TypeScript types and server infrastructure from the process
// definition in process.ts.
//
// Outputs:
...
// Run gentschema with the process definition piped via stdin.
const repoRoot = join(import.meta.dirname, '../..')
const result = spawnSync('go', ['run', './cmd/gentschema'], {
  cwd: repoRoot,
  input: JSON.stringify(processDefinition),
  encoding: 'utf8',
})
if (result.status !== 0) {
...
const schemaFile = JSON.parse(result.stdout) as SchemaFile
const { process_input: processInput, tasks = {}, $defs: defs = {} } = schemaFile
...
if (processInput) {
  ...
  if (inputDef) typeSections.push(await compile(inputDef, 'ProcessInput', opts))
}
...
function buildServerFile(): string {
  ...
  for (const id of taskIds) {
    ...
    const outputType = task.output ? `${pascal}Output` : 'Record<string, unknown>'
